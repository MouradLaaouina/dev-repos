...
$sql .= " FROM ".MAIN_DB_PREFIX.$paies->table_element." as o";
	...
	if ($resql2) {
		...
		if($tmptotlastpayrolls > 0) {
			while ($z < $tmptotlastpayrolls) {
			    ...
			    $results['situation_f'] = $obj->u_situation_f ? $obj->u_situation_f : $obj->situation_f;
				$results['nbrenfants'] 	= $obj->u_nbrenfants ? $obj->u_nbrenfants : $obj->nbrenfants;
...
if ($resql) {
	...
	if ($conf->global->PAIEDOLIBARR_ENABLE_ADDSALARY && ($user->admin || $usercandcreatesalary>0) && $num>0) 
	...
	if($massaction == "createsalary" && ($usercandcreatesalary>0 || $user->admin)){
	    ...
	    print $form->formconfirm("list.php?id=".$id."&page=".$page.'&massaction=createsalary&str_toselect='.urldecode($toselect).'&periodyear='.urlencode($periodyear).'&periodmonth='.urlencode($periodmonth), $langs->trans('Confirmation'), $langs->trans('Confirm_CreatedAllSalary', $period), 'confirm_createsalary', $formquestion,  "yes", 0, 250, 580);
	}
	...
	if (!empty($arrayfields['o.description']['checked'])) {
		$totalarray['nbfield']++;
		print_liste_field_titre($arrayfields['o.description']['label'], $_SERVER["PHP_SELF"], "o.description", "", $param, 'align="left"', $sortfield, $sortorder, 'left ');
	}
	if (!empty($arrayfields['o.period']['checked'])) {
		$totalarray['nbfield']++;
	...
	while ($i < min($num, $limit)) {
		...
		if (!empty($arrayfields['o.description']['checked'])) {
			print '<td class="">';
				print dol_trunc($obj->description, 80);
			print '</td>';
		}
