...
export default function ClientSearchPage({ user, onClientSelect, onBack, onLogout }) {
      ...
      <div className="bg-green-600 text-white px-6 py-4 shadow-lg">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold">Recherche Client</h1>
          </div>
          <button
            onClick={onLogout}
            className="text-sm bg-green-700 px-4 py-2 rounded-lg hover:bg-green-800"
          >
            Déconnexion
          </button>
        </div>
            ...
            {filteredClients.length > 0 ? (
              filteredClients.map(client => (
                <button
                  ...
                  className="w-full bg-white rounded-xl p-4 shadow-sm border border-gray-200 hover:shadow-md hover:border-green-300 transition-all text-left"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      {client.town && (
                        <p className="text-sm text-gray-600">{client.town}</p>
                      )}
                    </div>
